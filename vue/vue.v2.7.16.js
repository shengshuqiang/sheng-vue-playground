...
(function (global, factory) {
  ...
  // 便于代码理解处理
  function SSUScaffold(){
    let level = 0;
    const watchers = [];
    const deps = [];
    const msgs = function(msg, ...args) {
      return ['SSU',`${'\t'.repeat(level)} ${msg}`, ...args];
    }
    return {
      pushLevel() {
        level++;
  ...
  const _ssu_scaffold = new SSUScaffold();
  ...
  function hasChanged(x, y) {
    ...
    console.log(..._ssu_scaffold.msgs(`hasChanged(x, y) ${x}和${y}${result ? '不等' : '相等' }`, {x, y}));
    return result;
  }
  ...
  var VNode = /** @class */ (function () {
    ...
    VNode.prototype.toString = function () {
      return `VNode(${this.tag}, key=${this.key}, attrs=${JSON.stringify(this.data?.attrs)}, domProps=${JSON.stringify(this.data?.domProps)}, [${(this.children||[]).join('、')}])`;
    }
  ...
  var Dep = /** @class */ (function () {
    function Dep(_ssu_key) {
      _ssu_scaffold.pushLevel();
      this._ssu_key = _ssu_key;
      console.log(..._ssu_scaffold.msgs(`🔯新建${this}`, this));
      _ssu_scaffold.pushDep(this);
    ...
    Dep.prototype.name = function () {
      return `被观察者Dep「${this._ssu_key}」`;
    }
    Dep.prototype.toString = function () {
      return `被观察者Dep(${this._ssu_key}, [${this.subs ? this.subs.map(sub => sub.name()).join('、') : ''}])`;
    }
    Dep.prototype.addSub = function (sub) {
      _ssu_scaffold.pushLevel();
      console.log(..._ssu_scaffold.msgs(`${this.name()}.addSub(${sub.name()})`));
      this.subs.push(sub);
      _ssu_scaffold.popLevel();
    };
    ...
    Dep.prototype.depend = function (info) {
      _ssu_scaffold.pushLevel();
      console.log(..._ssu_scaffold.msgs(`${this.name()}.depend(info「${info && info.key}」)`));
      if (Dep.target) {
    ...
    Dep.prototype.notify = function (info) {
      _ssu_scaffold.pushLevel();
      console.log(..._ssu_scaffold.msgs(`${this.name()}.notify(${info})`));
      ...
      for (var i = 0, l = subs.length; i < l; i++) {
        var sub = subs[i];
        console.log(..._ssu_scaffold.msgs(`${this.name()}通知观察者${i+1}/${subs.length} ${sub.name()}`));
        if (info) {
  ...
  function pushTarget(target) {
    ...
    Dep.target = target;
    console.log(..._ssu_scaffold.msgs(`pushTarget()，当前观察者为${Dep.target ? Dep.target.name() : '空'}`));
    _ssu_scaffold.popLevel();
  }
  function popTarget() {
    _ssu_scaffold.pushLevel();
    ...
    console.log(..._ssu_scaffold.msgs(`popTarget()，当前观察者为${Dep.target ? Dep.target.name() : '空'}`));
    _ssu_scaffold.popLevel();
  }
  ...
  var Observer = /** @class */ (function () {
    function Observer(value, shallow, mock) {
      ...
      this.value = value;
      console.log(..._ssu_scaffold.msgs(`新建${this.name()}`));
      this.shallow = shallow;
      this.mock = mock;
      // this.value = value
      this.dep = mock ? mockDep : new Dep(JSON.stringify(value));
      this.vmCount = 0;
    ...
    Observer.prototype.name = function () {
      return `Observer「${JSON.stringify(this.value)}」`;
    }
    /**
  ...
  function defineReactive(
    ...
    console.log(..._ssu_scaffold.msgs(`defineReactive() 定义${JSON.stringify(val)}中${key}的值为响应式访问描述符reactiveGetter/reactiveSetter`));
    var dep = new Dep(key);
    ...
    Object.defineProperty(obj, key, {
      ...
      get: function reactiveGetter() {
        _ssu_scaffold.pushLevel();
        console.log(..._ssu_scaffold.msgs(`vue._data.${key}访问器，别名reactiveGetter()`));
      ...
      set: function reactiveSetter(newVal) {
        _ssu_scaffold.pushLevel();
        console.log(..._ssu_scaffold.msgs(`vue._data.${key}设置器，别名reactiveSetter(${newVal})`));
        var value = getter ? getter.call(obj) : val;
  ...
  function renderMixin(Vue) {
    ...
    Vue.prototype._render = function () {
      console.log(..._ssu_scaffold.msgs(`Vue.prototype._render() 执行渲染组件`));
      var vm = this;
      var _a = vm.$options,
        render = _a.render,
  ...
  function lifecycleMixin(Vue) {
    Vue.prototype._update = function (vnode, hydrating) {
      console.log(..._ssu_scaffold.msgs(`DOM Diff Vue.prototype._update`, {vnode, hydrating}));
      var vm = this;
      var prevEl = vm.$el;
  ...
  function mountComponent(vm, el, hydrating) {
    console.log(..._ssu_scaffold.msgs(`mountComponent`));
    vm.$el = el;
  ...
  function flushSchedulerQueue() {
    _ssu_scaffold.pushLevel();
    console.log(..._ssu_scaffold.msgs(`flushSchedulerQueue() \n${queue.map((watcher, index) => `${index + 1}. ${watcher.expression}`).join('\n')}`));
  ...
  function queueWatcher(watcher) {
    _ssu_scaffold.pushLevel();
    console.log(..._ssu_scaffold.msgs('queueWatcher '));
    var id = watcher.id;
    if (has[id] != null) {
      return;
    }
  ...
  var Watcher = /** @class */ (function () {
    function Watcher(vm, expOrFn, cb, options, isRenderWatcher) {
      _ssu_scaffold.pushLevel();
      this._ssu_key = `${isFunction(expOrFn) ? expOrFn.name + '()' : expOrFn}`;
      console.log(..._ssu_scaffold.msgs(`⚛️新建${this}`, this));
      _ssu_scaffold.pushWatcher(this);
      recordEffectScope(
        this,
    ...
    Watcher.prototype.name = function () {
      return `观察者Watcher「${this._ssu_key}」`;
    }
    Watcher.prototype.toString = function() {
      return `观察者Watcher(${this._ssu_key}, [${(this.deps ? this.deps.join('、') : '')}])`;
    }
    /**
    ...
    Watcher.prototype.get = function () {
      _ssu_scaffold.pushLevel();
      console.log(..._ssu_scaffold.msgs(`Watcher.get() 准备执行函数${this._ssu_key}`));
      pushTarget(this);
      var value;
      var vm = this.vm;
      try {
    ...
    Watcher.prototype.addDep = function (dep) {
      _ssu_scaffold.pushLevel();
      console.log(..._ssu_scaffold.msgs(`${this.name()}.addDep(${dep.name()})`));
      var id = dep.id;
    ...
    Watcher.prototype.update = function () {
      _ssu_scaffold.pushLevel();
      console.log(..._ssu_scaffold.msgs(`${this.name()}.update()`));
      /* istanbul ignore else */
      if (this.lazy) {
        console.log(..._ssu_scaffold.msgs(`${this.name()}.update() this.dirty = true`));
        this.dirty = true;
      } else if (this.sync) {
        console.log(..._ssu_scaffold.msgs(`${this.name()}.update() this.run()`));
        this.run();
      } else {
        console.log(..._ssu_scaffold.msgs(`${this.name()}.update() queueWatcher(this)`));
        queueWatcher(this);
      }
      _ssu_scaffold.popLevel();
    };
    ...
    Watcher.prototype.run = function () {
      _ssu_scaffold.pushLevel();
      console.log(..._ssu_scaffold.msgs(`${this.name()}.run()`));
      if (this.active) {
        var value = this.get();
        if (
          value !== this.value ||
    ...
    Watcher.prototype.evaluate = function () {
      _ssu_scaffold.pushLevel();
      console.log(..._ssu_scaffold.msgs(`${this.name()}.evaluate()`));
      this.value = this.get();
      this.dirty = false;
      _ssu_scaffold.popLevel();
    };
    ...
    Watcher.prototype.depend = function () {
      _ssu_scaffold.pushLevel();
      console.log(..._ssu_scaffold.msgs(`${this.name()}.depend()`));
      var i = this.deps.length;
      while (i--) {
        this.deps[i].depend();
      }
  ...
  function proxy(target, sourceKey, key) {
    console.log(..._ssu_scaffold.msgs(`proxy() 设置data中key为${key}的值代理到数据Vue.${sourceKey}.${key}`));
    sharedPropertyDefinition.get = function proxyGetter() {
      console.log(..._ssu_scaffold.msgs(`vue.data.${key}访问器，别名proxyGetter() 访问vue.data.${key}，代理到vue.${sourceKey}.${key}`));
      return this[sourceKey][key];
    };
    sharedPropertyDefinition.set = function proxySetter(val) {
      console.log(..._ssu_scaffold.msgs(`vue.data.${key}设置器，别名proxySetter() 设置vue.data.${key}，代理到vue.${sourceKey}.${key}`));
      this[sourceKey][key] = val;
    };
    Object.defineProperty(target, key, sharedPropertyDefinition);
  }
  function initState(vm) {
    console.log(..._ssu_scaffold.msgs(`initState() 初始化状态 <<<<<<<`));
    var opts = vm.$options;
  ...
  function initData(vm) {
    console.log(..._ssu_scaffold.msgs(`initData() 初始化data <<<<<<<`));
    var data = vm.$options.data;
    data = vm._data = isFunction(data) ? getData(data, vm) : data || {};
    data._ssu_key = 'vm._data';
  ...
  function initComputed$1(vm, computed) {
    console.log(..._ssu_scaffold.msgs(`initComputed$1(vm, computed) 初始化计算属性数据 <<<<<<<`, computed));
    // $flow-disable-line
    ...
    console.log(..._ssu_scaffold.msgs(`initComputed$1(vm, computed) 初始化计算属性数据 >>>>>>>`));
  }
  ...
  function createComputedGetter(key) {
    return function computedGetter() {
      ...
      console.log(..._ssu_scaffold.msgs(`计算属性${key}访问器，别名computedGetter() 对应${watcher.name()}`));
      if (watcher) {
        if (watcher.dirty) {
          watcher.evaluate();
        }
  ...
  function initWatch(vm, watch) {
    console.log(..._ssu_scaffold.msgs(`initWatch$1(vm, watch) 初始化侦听属性数据 <<<<<<<`, watch));
  ...
  function Vue(options) {
    ...
    this._ssu_scaffold = _ssu_scaffold;
    this._init(options);
  }
  ...
  var ref = {
    ...
    update: function (oldVnode, vnode) {
      console.log(..._ssu_scaffold.msgs(`DOM Diff update(oldVnode, vnode) ${oldVnode.data.ref !== vnode.data.ref ? '' : '不'}调用registerRef注销旧节点注册新节点`, {oldVnode, vnode}));
  ...
  function sameVnode(a, b, desc = 'unknow') {
    const result = (
      a.key === b.key &&
    ...
    console.log(..._ssu_scaffold.msgs(`DOM Diff sameVnode(${desc}) ${result}`, {a, b}));
    return result;
  }
  ...
  function createPatchFunction(backend) {
    ...
    function createElm(
      ...
      console.log(..._ssu_scaffold.msgs(`DOM Diff createElm`, 
        {vnode,
        insertedVnodeQueue,
        parentElm,
        refElm,
      ...
      if (isDef(tag)) {
        ...
        console.log(..._ssu_scaffold.msgs(`DOM Diff createElm vnode.elm=${vnode.elm.outerHTML}`, vnode.elm));
        setScope(vnode);
    ...
    function createComponent(vnode, insertedVnodeQueue, parentElm, refElm) {
      console.log(..._ssu_scaffold.msgs(`DOM Diff createComponent`, {vnode, insertedVnodeQueue, parentElm, refElm}));
      var i = vnode.data;
      if (isDef(i)) {
    ...
    function insert(parent, elm, ref) {
      console.log(..._ssu_scaffold.msgs(`DOM Diff insert`, {parent, elm, ref}));
      if (isDef(parent)) {
        if (isDef(ref)) {
    ...
    function createChildren(vnode, children, insertedVnodeQueue) {
      console.log(..._ssu_scaffold.msgs(`DOM Diff createChildren`, {vnode, children, insertedVnodeQueue}));
      if (isArray(children)) {
        {
    ...
    function updateChildren(
      ...
      console.log(..._ssu_scaffold.msgs(`DOM Diff updateChildren(${oldCh}, ${newCh})`, {
        parentElm,
        oldCh,
        newCh,
        insertedVnodeQueue,
      ...
      while (oldStartIdx <= oldEndIdx && newStartIdx <= newEndIdx) {
        if (isUndef(oldStartVnode)) {
          ...
          console.log(..._ssu_scaffold.msgs(`DOM Diff updateChildren isUndef(oldStartVnode「${oldStartVnode}」), ++oldStartIdx`, {oldStartIdx, oldStartVnode}));
        } else if (isUndef(oldEndVnode)) {
          oldEndVnode = oldCh[--oldEndIdx];
          console.log(..._ssu_scaffold.msgs(`DOM Diff updateChildren isUndef(oldEndVnode「${oldEndVnode}」), --oldEndVnode`, {oldEndIdx, oldEndVnode}));
        } else if (sameVnode(oldStartVnode, newStartVnode, 'oldStartVnode, newStartVnode')) {
          console.log(..._ssu_scaffold.msgs(`DOM Diff updateChildren sameVnode(oldStartVnode「${oldStartVnode}」, newStartVnode「${newStartVnode}」), ++oldStartIdx, ++newStartIdx`, {oldStartVnode, newStartVnode}));
          patchVnode(
            oldStartVnode,
            newStartVnode,
            insertedVnodeQueue,
        ...
        } else if (sameVnode(oldEndVnode, newEndVnode, 'oldEndVnode, newEndVnode')) {
          console.log(..._ssu_scaffold.msgs(`DOM Diff updateChildren sameVnode(oldEndVnode「${oldEndVnode}」, newEndVnode「${newEndVnode}」), --oldEndIdx, --newEndIdx`, {oldEndVnode, newEndVnode}));
          patchVnode(
            oldEndVnode,
            newEndVnode,
            insertedVnodeQueue,
        ...
        } else if (sameVnode(oldStartVnode, newEndVnode, 'oldStartVnode, newEndVnode')) {
          // Vnode moved right
          console.log(..._ssu_scaffold.msgs(`DOM Diff updateChildren sameVnode(oldStartVnode「${oldStartVnode}」, newEndVnode「${newEndVnode}」), ++oldStartIdx, --newEndIdx`, {oldStartVnode, newEndVnode}));
          patchVnode(
            oldStartVnode,
            newEndVnode,
            insertedVnodeQueue,
          ...
          oldStartVnode = oldCh[++oldStartIdx];
          newEndVnode = newCh[--newEndIdx];
        } else if (sameVnode(oldEndVnode, newStartVnode, 'oldEndVnode, newStartVnode')) {
          // Vnode moved left
          console.log(..._ssu_scaffold.msgs(`DOM Diff updateChildren sameVnode(oldEndVnode「${oldEndVnode}」, newStartVnode「${newStartVnode}」), --oldEndIdx, ++newStartIdx`, {oldEndVnode, newStartVnode}));
          patchVnode(
            oldEndVnode,
            newStartVnode,
            insertedVnodeQueue,
          ...
          if (isUndef(idxInOld)) {
            // New element
            console.log(..._ssu_scaffold.msgs(`DOM Diff updateChildren isUndef(idxInOld「${idxInOld}」), ++newStartIdx`));
            createElm(
              newStartVnode,
              insertedVnodeQueue,
              parentElm,
            ...
            if (sameVnode(vnodeToMove, newStartVnode, 'vnodeToMove, newStartVnode')) {
              console.log(..._ssu_scaffold.msgs(`DOM Diff updateChildren sameVnode(vnodeToMove「${vnodeToMove}」, newStartVnode「${newStartVnode}」), ++newStartIdx`, {vnodeToMove, vnodeToMove, newStartVnode}));
              patchVnode(
                vnodeToMove,
                newStartVnode,
                insertedVnodeQueue,
              ...
              console.log(..._ssu_scaffold.msgs(`DOM Diff updateChildren !sameVnode(vnodeToMove「${vnodeToMove}」, newStartVnode「${newStartVnode}」), ++newStartIdx`, {vnodeToMove, newStartVnode}));
              createElm(
                newStartVnode,
                insertedVnodeQueue,
                parentElm,
      ...
      if (oldStartIdx > oldEndIdx) {
        console.log(..._ssu_scaffold.msgs(`DOM Diff updateChildren oldStartIdx > oldEndIdx`, {oldStartIdx, oldEndIdx}));
        ...
        console.log(..._ssu_scaffold.msgs(`DOM Diff updateChildren newStartIdx > newEndIdx`, {newStartIdx, newEndIdx}));
    ...
    function patchVnode(
      ...
      console.log(..._ssu_scaffold.msgs(`DOM Diff patchVnode(${oldVnode}, ${vnode})`, {oldVnode, vnode, insertedVnodeQueue,
        ownerArray,
        index,
        removeOnly}));
      if (oldVnode === vnode) {
      ...
      if (isDef(data) && isPatchable(vnode)) {
        for (i = 0; i < cbs.update.length; ++i) {
          console.log(..._ssu_scaffold.msgs(`DOM Diff patchVnode ${i} ${cbs.update[i].name}`, { func: cbs.update[i], oldVnode, vnode}));
    ...
    return function patch(oldVnode, vnode, hydrating, removeOnly) {
      console.log(..._ssu_scaffold.msgs(`DOM Diff patch`, {oldVnode, vnode, hydrating, removeOnly}));
      if (isUndef(vnode)) {
  ...
  function updateDirectives(oldVnode, vnode) {
    console.log(..._ssu_scaffold.msgs(`DOM Diff updateDirectives(oldVnode, vnode) ${oldVnode.data.directives || vnode.data.directives ? '' : '不'}调用_update更新指令`, {oldVnode, vnode}));
    if (oldVnode.data.directives || vnode.data.directives) {
      _update(oldVnode, vnode);
    }
  }
  function _update(oldVnode, vnode) {
    console.log(..._ssu_scaffold.msgs(`_update\n`, {oldVnode, vnode}));
    ...
    console.log(..._ssu_scaffold.msgs(`_update oldDirs, newDirs\n`), {oldDirs, newDirs});
    var dirsWithInsert = [];
  ...
  function updateAttrs(oldVnode, vnode) {
    ...
    for (key in attrs) {
      cur = attrs[key];
      old = oldAttrs[key];
      const isNotEqual = old !== cur;
      console.log(..._ssu_scaffold.msgs(`DOM Diff updateAttrs old !== cur = ${(isNotEqual)},${(isNotEqual ? '' : '不')}调用el.setAttribute(key, value)更新属性`));
      if (isNotEqual) {
    ...
    for (key in oldAttrs) {
      if (isUndef(attrs[key])) {
        console.log(..._ssu_scaffold.msgs(`DOM Diff updateAttrs 调用elm.removeAttribute(key)移除额外属性${attrs[key]}`));
        if (isXlink(key)) {
  ...
  function updateClass(oldVnode, vnode) {
    console.log(..._ssu_scaffold.msgs(`DOM Diff updateClass(oldVnode, vnode)`, {oldVnode, vnode}));
    var el = vnode.elm;
    var data = vnode.data;
    ...
    if (cls !== el._prevClass) {
      console.log(..._ssu_scaffold.msgs(`DOM Diff updateClass 调用elm.setAttribute("class", cls)设置属性class为${cls}`));
  ...
  function add(name, handler, capture, passive) {
    ...
    console.log(..._ssu_scaffold.msgs(`DOM Diff add 调用el.addEventListener(name, handler)增加监听`, {name, handler}));
  ...
  function remove(name, handler, capture, _target) {
    console.log(..._ssu_scaffold.msgs(`DOM Diff remove 调用el.removeEventListener(name, handler)移除监听`, {name, handler: handler._wrapper || handler}));
  ...
  function updateDOMListeners(oldVnode, vnode) {
    console.log(..._ssu_scaffold.msgs(`DOM Diff updateDOMListeners(oldVnode, vnode)`, {oldVnode, vnode}));
  ...
  function updateDOMProps(oldVnode, vnode) {
    console.log(..._ssu_scaffold.msgs(`DOM Diff updateDOMProps(oldVnode, vnode)`, {oldVnode, vnode}));
    ...
    for (key in props) {
      ...
      if (key === "textContent" || key === "innerHTML") {
        ...
        if (elm.childNodes.length === 1) {
          console.log(..._ssu_scaffold.msgs(`DOM Diff updateDOMProps 调用elm.removeChild(elm.childNodes[0])移除子节点`, {childNode: elm.childNodes}));
      ...
      if (key === "value" && elm.tagName !== "PROGRESS") {
        ...
        if (shouldUpdateValue(elm, strCur)) {
          console.log(..._ssu_scaffold.msgs(`DOM Diff updateDOMProps 调用elm.value = strCur设置值`, {strCur}));
          elm.value = strCur;
        }
      } else if (
        key === "innerHTML" &&
        ...
        try {
          console.log(..._ssu_scaffold.msgs(`DOM Diff updateDOMProps 调用elm[key] = cur设置值`, {key, cur}));
          elm[key] = cur;
        } catch (e) {}
      }
    }
  ...
  var setProp = function (el, name, val) {
    console.log(..._ssu_scaffold.msgs(`DOM Diff setProp 调用el.style.setProperty(name, val)设置样式`, {name, val}));
    /* istanbul ignore if */
  ...
  function updateStyle(oldVnode, vnode) {
    console.log(..._ssu_scaffold.msgs(`DOM Diff updateStyle(oldVnode, vnode)`, {oldVnode, vnode}));
  ...
  function createCompileToFunctionFn(compile) {
    ...
    return function compileToFunctions(template, options, vm) {
      ...
      var res = {};
      var fnGenErrors = [];
      console.log(..._ssu_scaffold.msgs(`compiled.render源代码\n`, compiled.render));
      (function(rawCode){
        const short2nameObj = {
          _c: 'createElement',
          _o: 'markOnce',
        ...
        let readableCode = rawCode;
        // 将简写方法名换成可读方法名
        Object.keys(short2nameObj).forEach((key) => {
          readableCode = readableCode.replaceAll(key, short2nameObj[key]);
        })
        // 代码格式化缩进
        readableCode = (function formatCode(code) {
          let result = '';
          let indentLevel = 0;
          const indentSize = 2; // 缩进大小（空格数）
          ...
          for (let i = 0; i < code.length; i++) {
            ...
            if (char === '{' || char === '(' || char === '[') {
              result += char + '\n';
              indentLevel++;
              result += ' '.repeat(indentSize * indentLevel);
            } else if (char === '}' || char === ')' || char === ']') {
              result = result.trimEnd(); // 去掉当前行末尾的空格
              result += '\n';
              indentLevel--;
              result += ' '.repeat(indentSize * indentLevel) + char;
            } else if (char === ',') {
              result += char + '\n';
              result += ' '.repeat(indentSize * indentLevel);
            } else if (char === '\n') {
        ...
        console.log(..._ssu_scaffold.msgs(`compiled.render可读性代码\n`, readableCode));
      })(compiled.render);
